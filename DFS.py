"""
DFS Maze Visualizer (Tkinter)

Features:
- Generates a random maze (recursive backtracker) up to 200x200.
- Runs only the DFS algorithm and animates how it explores:
  - Open set (stack/frontier) cells in pale blue.
...
class DFSVisualizer:
    ...
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("DFS Maze Visualizer")
        ...
        self.solve_btn = tk.Button(panel, text="Solve (DFS)", command=self.on_solve)
    ...
    def on_solve(self) -> None:
        ...
        self._draw_maze()
        self.info_status.set("Status: running DFS")
        ...
        self._gen = self._dfs_steps(self.grid, self.start, self.end)
        self._running = True
        self._paused = False
    ...
    def _draw_maze(self) -> None:
        ...
        # Draw internal walls based on bitmasks: draw only East and South walls
        for r in range(rows):
            for c in range(cols):
                cell = self.grid[r][c]
    ...
    def _fill_cell(self, r: int, c: int, color: str, tag: str) -> None:
        self.canvas.create_rectangle(*self._cell_rect(r, c), fill=color, outline="", tags=(tag,))
    ...
    def _draw_current(self, r: int, c: int) -> None:
        ...
        pad = (x1 - x0) * 0.12
        self._current_item = self.canvas.create_rectangle(x0 + pad, y0 + pad, x1 - pad, y1 - pad,
                                                          fill=self.COLOR_CURRENT, outline="",
                                                          tags=("current",))
    ...
    def _dfs_steps(self, grid: List[List[int]], start: Tuple[int, int], end: Tuple[int, int]):
        rows, cols = len(grid), len(grid[0])
        self._cols = cols
        total = rows * cols
        ...
        while stack:
            ...
            def try_push(ni: int) -> None:
                if not visited[ni] and not in_stack[ni]:
                    prev[ni] = i
                    stack.append(ni)
                    in_stack[ni] = 1
                    _open_add.append(ni)

            # Push neighbors; order can influence the traversal path.
            if c + 1 < cols and not (cell & E):
                try_push(i + 1)
            if c - 1 >= 0 and not (cell & W):
                try_push(i - 1)
