"""
BFS Maze Visualizer (Tkinter)

Features:
- Generates a random maze (recursive backtracker) up to 200x200.
- Runs only the BFS algorithm and animates how it explores:
  - Open set (queue/frontier) cells in pale blue.
...
class BFSVisualizer:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.solve_btn = tk.Button(panel, text="Solve (BFS)", command=self.on_solve)
    ...
    def on_solve(self) -> None:
        ...
        self._draw_maze()
        self.info_status.set("Status: running BFS")
        ...
        self._gen = self._bfs_steps(self.grid, self.start, self.end)
        self._running = True
        self._paused = False
    ...
    def _bfs_steps(self, grid: List[List[int]], start: Tuple[int, int], end: Tuple[int, int]):
        rows, cols = len(grid), len(grid[0])
        self._cols = cols
        total = rows * cols
        ...
        while q:
            ...
            def try_enqueue(ni: int) -> None:
                if not visited[ni] and not in_queue[ni]:
                    prev[ni] = i
                    q.append(ni)
                    in_queue[ni] = 1
                    _open_add.append(ni)
